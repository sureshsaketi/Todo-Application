...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    response.send(responseData.map((eachItem) => outputResults(eachItem)));
  } else {
    response.status(400);
...
app.put("/todos/:todoId/", async (request, response) => {
  ...
  //   const previousTodoResponse = await database.get(previousTodoQuery);
  //   const {
  ...
  switch (true) {
    ...
    case requestBody.todo !== undefined:
      ...
      const changeTodo = await database.run(changeTodo);
